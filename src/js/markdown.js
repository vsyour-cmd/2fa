...
function embedSecretTokens(source) {
  const values = [];
  const replaced = source.replace(SECRET_PATTERN, (_, password) => {
...
function injectSecretRevealMarkup(html, values) {
...
export function renderMarkdown(value) {
  ...
  const { replaced, values } = embedSecretTokens(source);
  ...
  return injectSecretRevealMarkup(sanitized, values);
}
