...
- **使用场景**：记录操作流程，按使用顺序关联多个 2FA 条目，并在执行时直接查看和复制实时验证码
...
- **管理后台**：独立管理员登录、用户状态与备注管理、最近 20 个加密历史版本恢复、可恢复的保险库重置，以及 90 天操作日志查询；普通后台不提供永久删除用户功能
...
4. 普通后台不提供永久删除用户功能；需要停止访问时请停用用户，需要重新初始化时使用带 30 天密文归档的“重置保险库”
5. 老用户会在升级后的下一次在线登录或保存时上报账户标识；此前以“待识别用户”显示

### 主题与设置

- 点击右上角月亮图标可一键切换浅色/深色主题；点击齿轮进入设置后，还可选择「跟随系统」
- 设置中可配置排序、无操作自动锁定、进入后台立即锁定、复制后自动清空剪贴板、复制震动和回收站保留天数
- 可选的 6 位 PIN 只用于当前标签页锁定后 5 分钟内快速恢复；普通刷新由本机安全会话恢复，新标签页或 PIN 超时后仍需主密码
...
**导出备份**：可选择密码加密 JSON（推荐）、明文 JSON 或 OTPAuth URI 列表。完整 JSON 备份包含密钥、使用场景及二者的关联关系；明文格式含原始密钥，必须妥善保管。
...
3. **会话安全**：`sessionStorage` 不再保存原始解锁密钥，只保留会话元数据和随机引用；真正的密钥以不可导出的 `CryptoKey` 保存在 IndexedDB，最长 24 小时并在锁定时清理；仍建议启用自动锁定
